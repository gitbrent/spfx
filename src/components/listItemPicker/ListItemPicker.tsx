...
const strings = {
	'genericNoResultsFoundText': 'No Accounts Found',
	'ListItemPickerSelectValue': 'Enter Account Name'
};
...
export class ListItemPicker extends React.Component<IListItemPickerProps, IListItemPickerState> {
	 ...
	 */
	private loadListItems = async (filterText: string, listFilter: string): Promise<{ key: string; name: string }[]> => {
		...
		try {
			...
			let listItems = await sp.web.lists
				.getByTitle('Accounts').items
				.select('Title')
				.filter("startswith(" + columnInternalName +",'" + filterText + "')" + (listFilter ? " and "+listFilter : ""))
				.get()
				.then(web => {
					return web;
				});
